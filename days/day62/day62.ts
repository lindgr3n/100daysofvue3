class BoardManager {
    ...
    getBoardById({ x, y }: { x: number, y: number }): BoardCell {
...
class GameManager {
    ...
    onClick(evt) {
        ...
        // Check win condition
        const victory = this.winConditon.find(cords => {
            return cords.every(cord => this.boardManager.getBoardById({ x: cord[0], y: cord[1] }).getClickedBy() === player)
        })
        console.log('VICTORY', victory)
        if (victory) {
            victory.forEach((cord => {
                const cell: BoardCell = this.boardManager.getBoardById({ x: cord[0], y: cord[1] })
